...
OLD_MODEL_DIR = os.path.join(os.path.dirname(__file__), "../old_models")
...
def test_models_accuracy(train_model):
    """保存済み過去モデルと現在モデルの精度を比較"""
    ...
    # 過去モデルをすべて探索
    past_model_files = [
        f for f in os.listdir(OLD_MODEL_DIR)
        if f.endswith(".pkl") and f != "titanic_model.pkl"
    ]

    for past_file in past_model_files:
        past_path = os.path.join(OLD_MODEL_DIR, past_file)
        with open(past_path, "rb") as f:
            past_model = pickle.load(f)

        # 過去モデルの予測と精度
        past_pred = past_model.predict(X_test)
        past_acc = accuracy_score(y_test, past_pred)
